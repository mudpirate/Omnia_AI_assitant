...
// 🔥 DEDUPLICATE PRODUCTS - Remove duplicates based on title and price
function deduplicateProducts(products) {
  ...
  for (const product of products) {
    // Create deduplication key from title + price
    const title = product.title.toLowerCase().trim();
    const price = parseFloat(product.price);

    // Create a key combining title and price (rounded to 2 decimals)
...
async function executeSearchDatabase(args) {
  ...
  try {
    ...
    // 🔥 DEDUPLICATE RESULTS before slicing
...
"I found several iPhone 17 models available at Eureka and Best. The prices range from 274.9 to 369.9 KWD.
